...
Section br_language_mixin.
  ...
  Record BrLanguageMixin := {
    ...
    mixin_head_ctx_step_val K e σ1 κ e2 σ2 efs :
      head_step (fill K e) σ1 κ e2 σ2 efs → is_Some (to_val e) ∨ is_br e \/ K = empty_ectx;
  }.
End br_language_mixin.

Structure brLanguage := BrLanguage {
  expr : Type;
  val : Type;
  ectx : Type;
  state : Type;
  ...
  br_language_mixin :
    BrLanguageMixin of_val to_val is_br empty_ectx comp_ectx fill head_step
}.

Bind Scope expr_scope with expr.
Bind Scope val_scope with val.

Global Arguments BrLanguage {_ _ _ _ _ _ _ _ _ _ _} _.
Global Arguments of_val {_} _.
Global Arguments to_val {_} _.
Global Arguments is_br {_} _.
...
Section br_language.
  ...
  Proof. apply br_language_mixin. Qed.
  Lemma br_head_stuck e1 σ1 κ e2 σ2 efs : head_step e1 σ1 κ e2 σ2 efs → is_br e1 = false.
  Proof. apply br_language_mixin. Qed.
  ...
  Definition br_lang_mixin : LanguageMixin of_val to_val prim_step.
  Proof.
    split.
    - apply br_language_mixin.
    - apply br_language_mixin.
    - intros ?????? [??? -> -> ?%val_head_stuck]. (* TODO: Figure out this syntax *)
      apply eq_None_not_Some. by intros ?%fill_val%eq_None_not_Some.
  Qed.

  Canonical Structure br_lang : language := Language br_lang_mixin.
...
(* This will be broken without further spec on br in the language.  
  ...
  Proof.
    ...
    destruct (head_ctx_step_val _ _ _ _ _ _ _ Hred') as [[]%not_eq_None_Some|[Br|HK'']].
...
*)
  Lemma head_prim_step e1 σ1 κ e2 σ2 efs :
    head_step e1 σ1 κ e2 σ2 efs → prim_step e1 σ1 κ e2 σ2 efs.
  Proof. apply Br_step with empty_ectx; by rewrite ?fill_empty. Qed.
...
Global Arguments br_lang : clear implicits.
Global Arguments Br_step {Λ e1 σ1 κ e2 σ2 efs}.
Coercion br_lang : brLanguage >-> language.
...
Definition LanguageOfEctx (Λ : brLanguage) : language :=
  let '@BrLanguage E V C St K of_val to_val is_br empty comp fill head mix := Λ in
  @Language E V St K of_val to_val _
    (@br_lang_mixin (@BrLanguage E V C St K of_val to_val is_br empty comp fill head mix)).
